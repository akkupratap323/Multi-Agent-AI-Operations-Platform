...
const GITHUB_REPOS = ['Terrorizer-AI/opentelemetry-js'];
  ...
  try {
    ...
    if (messages.length > 0) {
      ...
      for (const message of messages.slice(0, 3)) {
        ...
        summary += `     - ${fromName}: ${subject.substring(0, 45)}...\n`;
      }
    }

    return summary;
  } catch (error) {
    return `📧 Email: Unable to check`;
  }
}
...
function getGitHubSummary() {
  ...
  try {
    // Check open PRs
    const openPRsData = runCommand(`gh pr list --repo ${GITHUB_REPOS[0]} --state open --json number`);
    let openPRs = 0;
    if (openPRsData) {
    ...
    const mergedData = runCommand(`gh pr list --repo ${GITHUB_REPOS[0]} --state merged --json mergedAt --limit 50`);
    let mergedToday = 0;
    if (mergedData) {
    ...
    // Check CI failures
    const runsData = runCommand(`gh run list --repo ${GITHUB_REPOS[0]} --limit 10 --json conclusion`);
    let failedBuilds = 0;
    if (runsData) {
...
async function getNeonDBSummary() {
  let summary = `💾 *Database Status*\n`;

  try {
    const projects = await fetchNeonAPI('/projects');
    ...
    const activeProjects = projects.projects.filter(p => {
      ...
      return updated.getTime() > oneWeekAgo;
    });

    summary += `   • ${projects.projects.length} projects (${activeProjects.length} active)\n`;

    // Check Ultron specifically
    const ultron = projects.projects.find(p => p.name === 'Ultron');
    if (ultron) {
      const endpoints = await fetchNeonAPI(`/projects/${ultron.id}/endpoints`);

      if (endpoints && endpoints.endpoints) {
        const activeEndpoints = endpoints.endpoints.filter(e => e.current_state === 'active');
        const idleEndpoints = endpoints.endpoints.filter(e => e.current_state === 'idle');

        if (activeEndpoints.length > 0) {
          summary += `   • Ultron: ${activeEndpoints.length} active endpoint(s)\n`;
        } else if (idleEndpoints.length > 0) {
          summary += `   • Ultron: All endpoints idle\n`;
        }
      }

      // Check recent operations (last 24h)
      const operations = await fetchNeonAPI(`/projects/${ultron.id}/operations`);
      if (operations && operations.operations) {
        const yesterday = Date.now() - 24 * 60 * 60 * 1000;
        const recentOps = operations.operations.filter(op => {
          return new Date(op.created_at).getTime() > yesterday;
        });

        if (recentOps.length > 0) {
          const computeOps = recentOps.filter(op =>
            op.action === 'start_compute' || op.action === 'suspend_compute'
          ).length;

          if (computeOps > 0) {
            summary += `   • ${computeOps} database operation(s) yesterday\n`;
          }
        }
      }
    }
...
async function main() {
  console.log('🚀 Generating complete morning briefing...\n');
  ...
  const neondbSummary = await getNeonDBSummary();
...
Have a productive day! 🚀`;
  ...
  console.log('\n✨ Complete morning briefing sent!');
}
